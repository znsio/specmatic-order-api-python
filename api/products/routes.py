...
if TYPE_CHECKING:
    from api.models import Id, Product, ProductType
...
def get_products():
    args: dict[str, ProductType | None] = new_prod_schema.load(request.args, partial=True)  # type: ignore[reportAssignmentType]
    if not args.get("type"):
        return prod_schema.dump(Database.all_products(), many=True)
    return prod_schema.dump(Database.find_products(args["type"]), many=True)
